...
export default function AgencyAdminReg() {
  const router = useRouter()
  const guid = useParams().uniqueguid
  ...
  const handleRegistration = async () => {
    ...
    if (registrationData.password === registrationData.confirmPassword) {
      try {
        ...
        await axios.post(
          `${process.env.NEXT_BASEURL}Auth/registerAgencyAdmin?agencyId=${guid}`,
          registrationData,
          requestOptions
        )
        .then((response) => {
  ...
  return (
    ...
    <Card color="transparent" shadow={false}>
      <Typography variant="h4" color="blue-gray">
        Sign Up as Agency Desk Officer
      </Typography>
